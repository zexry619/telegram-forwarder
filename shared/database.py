...
async def init_db():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # Tambahan kolom baru jika database lama belum memiliki kolom ini
...
async def get_user_config(user_id: int):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT target_chat_id, excluded_chat_ids, allowed_media_types, start_time, stop_time, reupload_on_restricted, eager_cache_enabled, status FROM users WHERE user_id = ?",
            (user_id,)
        ) as cursor:
            ...
            if not row:
                ...
                return {
                    ...
                    'eager_cache_enabled': False,
                    'status': 'stopped'
                }
            target, excluded_json, allowed_json, start_time, stop_time, reupload_flag, eager_cache_flag, status = row
            ...
            return {
                ...
                'stop_time': stop_time,
                'reupload_on_restricted': bool(reupload_flag or 0),
...
async def update_user_config(user_id: int, key: str, value):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if key in ['reupload_on_restricted', 'eager_cache_enabled']:
            value = 1 if value else 0
        allowed_keys = ['target_chat_id', 'excluded_chat_ids', 'allowed_media_types', 'start_time', 'stop_time', 'reupload_on_restricted', 'eager_cache_enabled', 'status', 'last_error']
...
async def db_check_duplicate_by_fingerprint(user_id: int, fp: str) -> str | None:
    ...
    sql = "SELECT message_key FROM forwarded_messages WHERE user_id = ? AND fingerprint = ? AND (status LIKE 'forwarded%' OR status LIKE 'reuploaded%' OR status LIKE 'duplicate%') LIMIT 1"
    async with aiosqlite.connect(DB_PATH) as db:
...
async def db_check_duplicate_by_thumbnail_hash(user_id: int, thumb_hash: str) -> str | None:
    ...
    sql = "SELECT message_key FROM forwarded_messages WHERE user_id = ? AND thumbnail_md5_hash = ? AND (status LIKE 'forwarded%' OR status LIKE 'reuploaded%' OR status LIKE 'duplicate%') LIMIT 1"
    async with aiosqlite.connect(DB_PATH) as db:
...
async def db_check_duplicate_by_image_hash(user_id: int, ih: str) -> str | None:
    ...
    sql = "SELECT message_key FROM forwarded_messages WHERE user_id = ? AND image_hash = ? AND (status LIKE 'forwarded%' OR status LIKE 'reuploaded%' OR status LIKE 'duplicate%') LIMIT 1"
    async with aiosqlite.connect(DB_PATH) as db:
...
async def db_check_duplicate_by_content_hash(user_id: int, ch: str) -> str | None:
    ...
    sql = "SELECT message_key FROM forwarded_messages WHERE user_id = ? AND content_hash = ? AND (status LIKE 'forwarded%' OR status LIKE 'reuploaded%' OR status LIKE 'duplicate%') LIMIT 1"
    async with aiosqlite.connect(DB_PATH) as db:
