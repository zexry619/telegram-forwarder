...
from .keyboards import main_menu_keyboard, onboarding_keyboard
...
def _is_cancelled(text: str | None) -> bool:
    return (text or "").strip().lower() in {"batal", "/cancel", "cancel"}
...
def setup_conversation_handlers(bot):
    ...
    async def login_handler(event):
        ...
        await event.edit(
            ...
            "1. Kirim nomor Telegram Anda, contoh `+62812xxxx`\n"
            "2. Masukkan kode verifikasi dari Telegram\n"
            "3. Jika akun Anda memakai 2FA, masukkan password\n\n"
        ...
        try:
            ...
            async with bot.conversation(user_id, timeout=300, exclusive=True) as conv:
                ...
                await conv.send_message("Masukkan nomor telepon Telegram Anda (format `+62...`).")
                phone = await conv.get_response()
                if _is_cancelled(phone.text):
                    await conv.send_message("Login dibatalkan.", buttons=onboarding_keyboard())
                    return
                ...
                try:
                    ...
                    await conv.send_message(
                        ...
                        "Masukkan kode dengan format `1_2_3_4_5`.\n"
                        "Contoh: jika kode Anda `12345`, kirim `1_2_3_4_5`."
                    )
                    code_response = await conv.get_response()
                    if _is_cancelled(code_response.text):
                        await conv.send_message("Login dibatalkan.", buttons=onboarding_keyboard())
                        return
                    ...
                    await conv.send_message("Akun Anda memakai 2FA. Masukkan password Telegram Anda:")
                    pw = await conv.get_response()
                    if _is_cancelled(pw.text):
                        await conv.send_message("Login dibatalkan.", buttons=onboarding_keyboard())
                        return
                ...
                await conv.send_message(
                    "✅ Login berhasil!\n\n"
                    "Langkah berikutnya:\n"
                    "1. Atur `Target Route Default` atau buka `Kelola Routes`\n"
                    "2. Atur pengecualian/filter default bila perlu\n"
                    "3. Jika perlu salin media lama, buka `Migrasi Media`\n"
                    "4. Klik `Jalankan` untuk mulai worker",
            ...
            await event.respond("Waktu habis. Proses login dibatalkan.", buttons=onboarding_keyboard())
        except Exception as e:
            logger.error(f"Login failed for user {user_id}", exc_info=True)
            await event.respond(
                f"❌ Terjadi error saat login: {type(e).__name__}\nSilakan coba lagi dari tombol login.",
                buttons=onboarding_keyboard()
            )
        finally:
